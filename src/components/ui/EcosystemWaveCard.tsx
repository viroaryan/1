...
interface EcosystemWaveCardProps {
  title: string;
  subtitle: string;
  desc: string;
  status: string;
  href: string;
  theme?: "default" | "emerald" | "amber" | "lavender";
  iconSvg?: React.ReactNode;
}

export default function EcosystemWaveCard({
  title,
  subtitle,
  desc,
  status,
  ...
  return (
    <Link
      href={href}
      className={`e-card playing ${themeClass} group`}
    ...
    >
      ...
      <div className="infotop">
        {/* Centered Icon */}
        <div className="icon flex items-center justify-center mx-auto text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.35)]">
          {iconSvg || (
            <svg
        ...
        <div className="card-title text-xl font-bold tracking-tight text-white drop-shadow-sm px-4 mt-1">
          {title}
        </div>

        {/* Tagline / Role Subtitle */}
        <div className="name text-xs font-medium text-white/90 px-4 mt-0.5">
          {subtitle}
        </div>

        {/* Status Pill Badge */}
        <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/30 backdrop-blur-md border border-white/20 text-[10px] font-mono tracking-wider uppercase text-white/95 shadow-xs">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
          <span>{status}</span>
        </div>

        {/* Description */}
        <p className="mt-3 px-5 text-[11px] text-white/80 leading-relaxed line-clamp-2">
          {desc}
        </p>

        {/* Floating Learn More Indicator */}
        <div className="mt-4 inline-flex items-center gap-1 text-[11px] font-mono font-medium text-white/90 group-hover:text-white group-hover:translate-x-1 transition-all">
